...
# 1. we must iterate over existing role and create a different assume role policy
# for each of them 

#2. In each role policy, add under identifiers add only the users that have that 
